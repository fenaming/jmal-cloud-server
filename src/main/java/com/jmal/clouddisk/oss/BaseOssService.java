...
public class BaseOssService {
    ...
    /**
     * 上传ID缓存, 24小时过期防止未完成的分片上传导致内存泄漏
     */
    private final Cache<String, String> updateIdCache;
    ...
    public BaseOssService(IOssService ossService, String bucketName, FileProperties fileProperties, ScheduledThreadPoolExecutor scheduledThreadPoolExecutor, OssConfigDTO ossConfigDTO) {
        ...
        this.tempFileCache = Caffeine.newBuilder().expireAfterWrite(1, TimeUnit.HOURS).build();
        this.tempFileListCache = Caffeine.newBuilder().expireAfterWrite(1, TimeUnit.HOURS).build();
        this.waitingUploadCache = Caffeine.newBuilder().expireAfterWrite(1, TimeUnit.HOURS).build();
        this.updateIdCache = Caffeine.newBuilder().expireAfterWrite(24, TimeUnit.HOURS).build();
    ...
    public String getUploadId(String objectName) {
        String uploadId = updateIdCache.getIfPresent(objectName);
        if (uploadId == null) {
     ...
     * 该方法每秒执行一次 <br/>
     * 注意: 必须捕获所有异常, 否则 ScheduledExecutorService 会在未捕获异常后静默停止调度
     */
    private void checkUpload() {
        try {
            ...
            log.error("checkUpload error: {}", e.getMessage(), e);
        }
    }
