...
public class WebOssCopyFileService extends WebOssCommonService {
     ...
     * @param isMove               是否为移动操作
     */
    public ResponseResult<Object> copyOssToLocal(String ossPathFrom, String sourceObjectNamePath, String fileId, boolean isMove, Boolean isTogether, String spaceFlag) {
        IOssService ossServiceFrom = OssConfigService.getOssStorageService(ossPathFrom);
        String objectNameFrom = sourceObjectNamePath.substring(ossPathFrom.length());
        boolean isFolder = objectNameFrom.endsWith("/");
        ...
        if (isFolder) {
            // 复制文件夹
            copyDir(ossServiceFrom, objectNameFrom, destFileDocument, isTogether, spaceFlag);
        } else {
            // 复制文件
     ...
     */
    private void copyDir(IOssService ossServiceFrom, String objectNameFrom, FileDocument destFileDocument, Boolean isTogether, String spaceFlag) {
        // 锁对象
        ossServiceFrom.lock(objectNameFrom);
        try {
            String destDir = null;
            if(Boolean.TRUE.equals(isTogether)){
                destDir = Paths.get(fileProperties.getRootDir(), fileProperties.getTogetherFileDir(), destFileDocument.getPath(), destFileDocument.getName()).toString();
            }else if (StringUtils.isNotEmpty(spaceFlag)){
                destDir = Paths.get(fileProperties.getRootDir(), spaceFlag, destFileDocument.getPath(), destFileDocument.getName()).toString();
            }else{
                destDir = Paths.get(fileProperties.getRootDir(), destFileDocument.getUsername(), destFileDocument.getPath(), destFileDocument.getName()).toString();
            }
            ...
            fileInfoList.stream().filter(FileInfo::isFolder).parallel().forEach(fileInfo -> {
                ...
                commonFileService.createFile(destFileDocument.getUsername(), destPath.toFile(), null, null, isTogether, spaceFlag);
            });
            // 再复制文件
            fileInfoList.stream().filter(fileInfo -> !fileInfo.isFolder()).parallel().forEach(fileInfo -> {
                String relativePath = fileInfo.getKey().substring(objectNameFrom.length());
                ...
                try (AbstractOssObject abstractOssObject = ossServiceFrom.getAbstractOssObject(fileInfo.getKey());
                    ...
                    commonFileService.createFile(destFileDocument.getUsername(), destFile, null, null, isTogether, spaceFlag);
                } catch (Exception e) {
                    throw new CommonException(ExceptionType.SYSTEM_ERROR.getCode(), e.getMessage());
                }
            });
    ...
    private void copyFile(IOssService ossServiceFrom, String objectNameFrom, FileDocument destFileDocument) {
        ...
        try (AbstractOssObject abstractOssObject = ossServiceFrom.getAbstractOssObject(objectNameFrom);
            ...
            FileUtil.writeFromStream(inputStream, destFile);
            commonFileService.createFile(destFileDocument.getUsername(), destFile, null, null, destFileDocument.getIsTogether(), destFileDocument.getSpaceFlag());
        } catch (Exception e) {
            throw new CommonException(ExceptionType.SYSTEM_ERROR.getCode(), e.getMessage());
        } finally {
            // 解锁对象
