...
public class VideoController {
    ...
    @GetMapping("/together/video/hls/{username}/{fileId}.m3u8")
    public ResponseEntity<UrlResource> m3u8Together(@PathVariable String username, @PathVariable String fileId) throws IOException {
        Path m3u8Path = Paths.get(fileProperties.getRootDir(), fileProperties.getChunkFileDir(), fileProperties.getTogetherFileDir(), fileProperties.getVideoTranscodeCache(), fileId, fileId + ".m3u8");
        UrlResource videoResource = new UrlResource(m3u8Path.toUri());
        return ResponseEntity.ok()
                .header(HttpHeaders.CONTENT_TYPE, "application/vnd.apple.mpegurl")
                .body(videoResource);
    ...
    @GetMapping("/together/video/hls/{username}/{fileId}-{index}.ts")
    public ResponseEntity<UrlResource> tsTogether(@PathVariable String username, @PathVariable String fileId, @PathVariable String index) throws IOException {
        Path m3u8Path = Paths.get(fileProperties.getRootDir(), fileProperties.getChunkFileDir(), fileProperties.getTogetherFileDir(), fileProperties.getVideoTranscodeCache(), fileId, fileId + "-" + index + ".ts");
        UrlResource videoResource = new UrlResource(m3u8Path.toUri());
        return ResponseEntity.ok()
                .header(HttpHeaders.CONTENT_TYPE, "application/vnd.apple.mpegurl")
                .body(videoResource);
    ...
    @GetMapping("/together/public/video/hls/{shareId}/{shareToken}/{username}/{fileId}.m3u8")
    public ResponseEntity<UrlResource> publicM3u8Together(@PathVariable String username, @PathVariable String fileId, @PathVariable String shareId, @PathVariable String shareToken) throws IOException {
        shareService.validShare(shareToken, shareId);
        return m3u8(username, fileId);
    }

    @GetMapping("/together/public/video/hls/{shareId}/{shareToken}/{username}/{fileId}-{index}.ts")
    public ResponseEntity<UrlResource> publicTsTogether(@PathVariable String username, @PathVariable String fileId, @PathVariable String index, @PathVariable String shareId, @PathVariable String shareToken) throws IOException {
        shareService.validShare(shareToken, shareId);
        return ts(username, fileId, index);
    }
}
