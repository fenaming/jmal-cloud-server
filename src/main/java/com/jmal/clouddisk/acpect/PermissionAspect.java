...
import com.jmal.clouddisk.annotation.AnnotationResolver;
...
public class PermissionAspect {
    ...
    public Object before(ProceedingJoinPoint joinPoint) throws Throwable {
        ...
        String authority = (String) AnnotationResolver.newInstance().resolver(joinPoint, permission.value());
//        String authority = permission.value();
        if (CharSequenceUtil.isBlank(authority)) {
            // 方法上没有权限注解, 直接调用方法
            return joinPoint.proceed();
        }
        // 上传前检查空间是否够用（个人空间和 Space 空间都需要检查）
        if (authority.endsWith(":file:upload") && CaffeineUtil.spaceFull(userLoginHolder.getUserId())) {
