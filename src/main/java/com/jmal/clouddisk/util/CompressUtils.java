...
public class CompressUtils {
    ...
    /**
     * Zip Slip 安全校验：确保解压的文件路径不会逃逸到目标目录之外
     *
     * @param destDir   目标解压目录
     * @param entryName 压缩条目名称
     * @return 安全的目标文件
     * @throws IOException 如果路径逃逸到目标目录之外
     */
    private static File validateEntryPath(String destDir, String entryName) throws IOException {
        ...
        if (!canonicalDestFile.startsWith(canonicalDestDir + File.separator) && !canonicalDestFile.equals(canonicalDestDir)) {
            throw new IOException("Zip Slip 安全检查失败，非法条目路径: " + entryName);
        }
        return destFile;
    }
        ...
        try (
            ...
            while (Objects.nonNull(entry = in.getNextEntry())) {
                if (in.canReadEntryData(entry)) {
                    // Zip Slip 防护
                    File file = validateEntryPath(destDir, entry.getName());
                    ...
                    log.warn("无法读取压缩条目: {}", entry.getName());
                }
            }
        }
    }
    ...
    public static void unZip(File file, String outputDir, boolean isWrite) throws IOException {
        try (ZipFile zipFile = new ZipFile(file, "utf-8")) {
            ...
            while (enums.hasMoreElements()) {
                ...
                // Zip Slip 防护
                File destFile = validateEntryPath(outputDir, entry.getName());
                if (entry.isDirectory()) {
                    //创建空目录
                    if (!destFile.exists()) {
                        destFile.mkdirs();
    ...
    public static void decompressTarGz(File file, String outputDir, boolean isWrite) throws IOException {
        ...
        try (TarArchiveInputStream tarIn = new TarArchiveInputStream(
            ...
            while ((entry = tarIn.getNextTarEntry()) != null) {
                decompressEntry(outputDir, isWrite, tarIn, entry);
            }
        }
    }
    ...
    public static void decompressJar(File file, String outputDir, boolean isWrite) throws IOException {
        //创建输出目录
        createDirectory(outputDir, null);
        try (JarArchiveInputStream inputStream = new JarArchiveInputStream(new FileInputStream(file))) {
            JarArchiveEntry entry;
            while (Objects.nonNull(entry = inputStream.getNextJarEntry())) {
                decompressEntry(outputDir, isWrite, inputStream, entry);
            }
        }
    }
    ...
    public static void decompressTar(File file, String outputDir, boolean isWrite) throws IOException {
        //创建输出目录
        createDirectory(outputDir, null);
        try (TarArchiveInputStream inputStream = new TarArchiveInputStream(new FileInputStream(file))) {
            TarArchiveEntry entry;
    ...
    /**
     * 解压单个条目，带 Zip Slip 防护和资源安全关闭
     */
    private static void decompressEntry(String outputDir, boolean isWrite, InputStream inputStream, ArchiveEntry entry) throws IOException {
        // Zip Slip 防护
        File destFile = validateEntryPath(outputDir, entry.getName());
        //是目录
        if (entry.isDirectory()) {
