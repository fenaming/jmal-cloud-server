...
public class CaffeineUtil {
    ...
    private static final long CHUNK_UPLOAD_CACHE_MAX_SIZE = 100_000L;
    private static final long CHUNK_UPLOAD_LOCK_CACHE_MAX_SIZE = 50_000L;
     ...
     * value: BucketInfo
     */
    private static final Cache<String, BucketInfo> OSS_DIAMETER_PREFIX_CACHE = Caffeine.newBuilder()
            .expireAfterAccess(2, TimeUnit.HOURS)
            .build();

    private static final Cache<String, Long> LAST_ACCESS_TIME_CACHE = Caffeine.newBuilder()
            .expireAfterWrite(1, TimeUnit.HOURS)
            .build();

    /***
     * 空间已满的用户
     * 添加过期策略，避免缓存过期后用户依然被标记为空间已满
     */
    private static final Cache<String, String> SPACE_FULL = Caffeine.newBuilder()
     ...
     */
    public static final Cache<String, ConsumerDO> CONSUMER_USERNAME = Caffeine.newBuilder()
     ...
     * 添加过期策略，确保权限变更后能及时生效
     */
    private static final Cache<String, List<String>> AUTHORITIES_CACHE = Caffeine.newBuilder()
            .expireAfterWrite(30, TimeUnit.MINUTES)
            .build();
     ...
     * value: userId
     */
    private static final Cache<String, String> USER_ID_CACHE = Caffeine.newBuilder()
            .expireAfterAccess(2, TimeUnit.HOURS)
            .build();
     ...
     * value: username
     */
    private static final Cache<String, String> USERNAME_CACHE = Caffeine.newBuilder()
            .expireAfterAccess(2, TimeUnit.HOURS)
            .build();
    ...
    public static void initMyCache(){
        if(resumeCache == null){
            // 分片上传状态缓存，24小时过期，防止放弃的上传占用内存
            resumeCache = Caffeine.newBuilder()
                    .maximumSize(CHUNK_UPLOAD_CACHE_MAX_SIZE)
                    .expireAfterWrite(24, TimeUnit.HOURS)
                    .build();
        }
        if(writtenCache == null){
            writtenCache = Caffeine.newBuilder()
                    .maximumSize(CHUNK_UPLOAD_CACHE_MAX_SIZE)
                    .expireAfterWrite(24, TimeUnit.HOURS)
                    .build();
        }
        if(unWrittenCache == null){
            unWrittenCache = Caffeine.newBuilder()
                    .maximumSize(CHUNK_UPLOAD_CACHE_MAX_SIZE)
                    .expireAfterWrite(24, TimeUnit.HOURS)
                    .build();
        }
        if(chunkWriteLockCache == null){
            chunkWriteLockCache = Caffeine.newBuilder()
                    .maximumSize(CHUNK_UPLOAD_LOCK_CACHE_MAX_SIZE)
                    .expireAfterWrite(24, TimeUnit.HOURS)
                    .build();
        }
