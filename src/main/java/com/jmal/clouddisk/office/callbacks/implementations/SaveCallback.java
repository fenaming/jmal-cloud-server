    ...
    public int handle(Track body) {
        ...
        try {
            ...
            if (Boolean.TRUE.equals(fileDocument.getIsTogether())){
                ...
                path = Paths.get(fileProperties.getRootDir(), userLoginHolder.getUsername(), fileDocument.getPath(), fileDocument.getName());
            }
            // 保存历史文件
            String relativePath = Paths.get(fileDocument.getPath(), fileDocument.getName()).toString();
            fileVersionService.saveFileVersion(userLoginHolder.getUsername(), relativePath, userLoginHolder.getUserId(), fileDocument.getIsTogether(), fileDocument.getSpaceFlag());
