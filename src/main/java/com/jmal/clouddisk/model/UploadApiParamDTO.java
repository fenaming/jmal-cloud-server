...
public class UploadApiParamDTO {
    ...
    public String getFilename() {
        String decoded = URLUtil.decode(filename);
        if (decoded != null && (decoded.contains("..") || decoded.contains("/") || decoded.contains("\\"))) {
            // 消毒文件名，只取最后一段文件名，防止路径遍历攻击
            decoded = java.nio.file.Paths.get(decoded).getFileName().toString();
        }
        return decoded;
    }
