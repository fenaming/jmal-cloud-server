...
public class RoleService {
    ...
    public ResponseResult<List<RoleDTO>> list(QueryRoleDTO queryDTO) {
        ...
        if(!CharSequenceUtil.isBlank(queryDTO.getCode())){
            query.addCriteria(Criteria.where("code").regex(ReUtil.escape(queryDTO.getCode()), "i"));
        }
        if(!CharSequenceUtil.isBlank(queryDTO.getRemarks())){
            query.addCriteria(Criteria.where("remarks").regex(ReUtil.escape(queryDTO.getRemarks()), "i"));
        }
    ...
    public List<String> getAuthorities(List<String> roleIdList) {
        ...
        for (RoleDO roleDO : roleDOList) {
            // 如果是超级管理员, 直接返回所有菜单中的权限标识
            // 注意: 不能使用 AnnoManageUtil.AUTHORITIES，因为其中包含原始注解表达式
            // 如 "#{upload.spaceFlag}:file:upload"，无法与解析后的动态权限匹配
