...
public class FileVersionServiceImpl implements IFileVersionService {
    ...
    @Override
    public void saveFileVersion(String username, String relativePath, String userId, Boolean isTogether, String spaceFlag) {
        File file = null;
        String filepath = null;
        if(Boolean.TRUE.equals(isTogether)){
            file = new File(Paths.get(fileProperties.getRootDir(), fileProperties.getTogetherFileDir(), relativePath).toString());
            filepath = Paths.get(fileProperties.getTogetherFileDir(), relativePath).toString();
        }else if(StringUtils.isNotEmpty(spaceFlag)){
            file = new File(Paths.get(fileProperties.getRootDir(), spaceFlag, relativePath).toString());
            filepath = Paths.get(spaceFlag, relativePath).toString();
        }else{
            file = new File(Paths.get(fileProperties.getRootDir(), username, relativePath).toString());
            filepath = Paths.get(username, relativePath).toString();
        }
        FileDocument fileDocument = commonFileService.getFileDocumentByPath(filepath, userId, isTogether, spaceFlag);
    ...
    @Override
    public ResponseResult<List<GridFSBO>> listFileVersionByPath(String path, Integer pageSize, Integer pageIndex, Boolean isTogether, String spaceFlag) {
        String fileId = null;
        String username = userLoginHolder.getUsername();
        String ownerPrefix = username;
        if (Boolean.TRUE.equals(isTogether)) {
            ownerPrefix = fileProperties.getTogetherFileDir();
        } else if (StringUtils.isNotEmpty(spaceFlag)) {
            ownerPrefix = spaceFlag;
        }
        Path prePth = Paths.get(ownerPrefix, path);
        ...
        if (ossPath != null) {
            ...
            FileDocument fileDocument = commonFileService.getFileDocumentByPath(relativePath, filePath.getFileName().toString(), userId, isTogether, spaceFlag);
    ...
    public Long recovery(String gridFSId) {
        ...
        if (Boolean.TRUE.equals(fileDocument.getIsTogether())){
            file = Paths.get(fileProperties.getRootDir(), fileProperties.getTogetherFileDir(), fileDocument.getPath(), filename).toFile();
        }else if (StringUtils.isNotEmpty(fileDocument.getSpaceFlag())){
            file = Paths.get(fileProperties.getRootDir(), fileDocument.getSpaceFlag(), fileDocument.getPath(), filename).toFile();
        }else{
            file = Paths.get(fileProperties.getRootDir(), username, fileDocument.getPath(), filename).toFile();
        }
        if (!file.exists()) {
            throw new CommonException(ExceptionType.FILE_NOT_FIND);
        }
        if (CommonFileService.isLock(file, fileProperties.getRootDir(), username, fileDocument.getIsTogether(), fileDocument.getSpaceFlag())) {
    ...
    public ResponseEntity<InputStreamResource> readHistoryFile(String gridFSId) {
        ...
        try {
            // 注意: 此处不能使用 try-with-resources, 因为 InputStream 需要保持打开状态
            // 由 Spring 的 InputStreamResource 在响应写完后自动关闭
            InputStream inputStream = getInputStream(gridFSFile);
            String filename = gridFSFile.getMetadata().getString(Constants.FILENAME);
    ...
    public static InputStream gzipCompress(InputStream inputStream, Metadata metadata) {
        ...
        try {
            ...
            }, "gridfs-gzip-compress");
            gzipThread.setDaemon(true);
            gzipThread.start();
     ...
     */
    public static InputStream gzipDecompress(InputStream inputStream, Document metadata) throws IOException {
