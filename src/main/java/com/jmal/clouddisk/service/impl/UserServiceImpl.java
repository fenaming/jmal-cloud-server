...
public class UserServiceImpl implements IUserService {
    ...
    public synchronized ConsumerDO add(ConsumerDTO consumerDTO) {
        ...
        if (fileProperties.getChunkFileDir().equals(username) || fileProperties.getTogetherFileDir().equals(username)
                || fileProperties.getDesignerFileDir().equals(username) || fileProperties.getMachineFileDir().equals(username)
    ...
    public ResponseResult<Object> update(ConsumerDTO user, MultipartFile blobAvatar) {
        ...
        if (fileProperties.getChunkFileDir().equals(name) || fileProperties.getTogetherFileDir().equals(name)
                || fileProperties.getDesignerFileDir().equals(name) || fileProperties.getMachineFileDir().equals(name)
                || fileProperties.getInstallFileDir().equals(name) || fileProperties.getControlFileDir().equals(name)
                || fileProperties.getElectricFileDir().equals(name) || fileProperties.getSampleFileDir().equals(name)
                || (StringUtils.isNotEmpty(name) && name.startsWith("space"))) {
    ...
    public ResponseResult<List<ConsumerDTO>> userList(QueryUserDTO queryDTO) {
        ...
        if (!CharSequenceUtil.isBlank(queryDTO.getUsername())) {
            query.addCriteria(Criteria.where(USERNAME).regex(ReUtil.escape(queryDTO.getUsername()), "i"));
        }
        if (!CharSequenceUtil.isBlank(queryDTO.getShowName())) {
            query.addCriteria(Criteria.where(SHOW_NAME).regex(ReUtil.escape(queryDTO.getShowName()), "i"));
        }
...
//            String originalPwd = "jmalcloud";
    ...
    public List<String> getAuthorities(String username) {
        ...
        // 如果是创建者, 直接返回所有菜单中的权限标识
        // 注意: 不能使用 AnnoManageUtil.AUTHORITIES，因为其中包含的是原始注解表达式
        // 如 "#{upload.spaceFlag}:file:upload"，无法与解析后的动态权限（如 "designer:file:upload"）匹配
        if (consumerDO.getCreator() != null && consumerDO.getCreator()) {
            return menuService.getAllMenuAuthorities();
        }
