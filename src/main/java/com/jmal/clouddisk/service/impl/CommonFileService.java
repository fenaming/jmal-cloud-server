...
public class CommonFileService {
     ...
     * @return FileDocument
     */
    FileDocument getByMd5(String path, String userId, String md5, Boolean isTogether, String spaceFlag) {
     ...
     * @return FileDocument
     */
    public FileDocument getFileDocumentByPath(String path, String filename, String userId, Boolean isTogether, String spaceFlag) {
        Query query = getQuery(path, filename, userId, isTogether, spaceFlag);
        return mongoTemplate.findOne(query, FileDocument.class, COLLECTION_NAME);
    }

    public static Query getQuery(FileDocument fileDocument) {
        return getQuery(fileDocument.getPath(), fileDocument.getName(), fileDocument.getUserId(), fileDocument.getIsTogether(), fileDocument.getSpaceFlag());
    }

    public static Query getQuery(String path, String name, String userId, Boolean isTogether, String spaceFlag) {
        Query query = new Query();
        if (Boolean.TRUE.equals(isTogether)){
            query.addCriteria(Criteria.where(IS_TOGETHER).is(Boolean.TRUE));
        }else if (StringUtils.isNotEmpty(spaceFlag)){
     ...
     * @return FileDocument
     */
    public FileDocument getFileDocumentByPath(String filepath, String userId, Boolean isTogether, String spaceFlag) {
        ...
        return getFileDocumentByPath(path, filename, userId, isTogether, spaceFlag);
    }
    ...
    public void createFolder(UploadApiParamDTO upload) {
        ...
        // 保存文件夹信息
        createFile(upload.getUsername(), dir, null, null, upload.getIsTogether(), upload.getSpaceFlag());
    }
     ...
     * @return fileId
     */
    public String createFile(String username, File file, String userId, Boolean isPublic, Boolean isTogether, String spaceFlag) {
        ...
        try {
            int startIndex = 0;
            if (Boolean.TRUE.equals(isTogether)){
                startIndex = fileProperties.getRootDir().length() + fileProperties.getTogetherFileDir().length() + 1;
            }else if (StringUtils.isNotEmpty(spaceFlag)){
                startIndex = fileProperties.getRootDir().length() + spaceFlag.length() + 1;
            }else{
                startIndex = fileProperties.getRootDir().length() + username.length() + 1;
            }
            ...
            Query query = new Query();
            FileDocument fileExists = getFileDocument(userId, fileName, relativePath, query, isTogether, spaceFlag);
            if (fileExists != null) {
            ...
            if (Boolean.TRUE.equals(isTogether)) {
                update.set("isTogether", isTogether);
            }
            if (StringUtils.isNotEmpty(spaceFlag)){
                update.set("spaceFlag", spaceFlag);
            }
            if (file.isFile()) {
                setFileConfig(fileId, username, file, fileName, suffix, contentType, relativePath, update, isTogether, spaceFlag);
            } else {
                // 检查目录是否为OSS目录
                checkOSSPath(username, relativePath, fileName, update, isTogether, spaceFlag);
            }
    ...
    public String createFileDocument(String username, File file, String userId, Boolean isPublic, Boolean isTogether, String spaceFlag) {
        if (CharSequenceUtil.isBlank(username)) {
            return null;
        }
        if (CharSequenceUtil.isBlank(userId)) {
...
//            pushMessage(username, update.getUpdateObject(), "createFile");
    ...
    public FileDocument getFileDocument(String userId, String fileName, String relativePath, Query query, Boolean isTogether, String spaceFlag) {
        if (Boolean.TRUE.equals(isTogether)){
            query.addCriteria(Criteria.where(IS_TOGETHER).is(Boolean.TRUE));
        }else if (StringUtils.isNotEmpty(spaceFlag)){
            query.addCriteria(Criteria.where(SPACE_FLAG).is(spaceFlag));
    ...
    public FileDocument getFileDocument(String username, String fileAbsolutePath, Boolean isTogether, String spaceFlag)  {
        ...
        return getFileDocument(userId, fileName, relativePath, new Query(), isTogether, spaceFlag);
    }

    /**
     * 跳过 WebP 转换的最大文件大小阈值（20MB）
     * 超大图片文件在 ImageIO.read() 时会将整个像素数据解压到堆内存，
     * 例如一张 5000x4000 的 ARGB 图片约占用 80MB 堆内存，
     * 多个用户同时上传大图可能导致 OOM。
     */
    private static final long MAX_IMAGE_SIZE_FOR_WEBP = 20L * 1024 * 1024;
    ...
    private File replaceWebp(String userId, File file) {
        ...
        // 跳过超大文件的 WebP 转换，避免 ImageIO.read() 将大图片解压到堆内存导致 OOM
        if (file.length() > MAX_IMAGE_SIZE_FOR_WEBP) {
            log.info("跳过 WebP 转换，文件过大: {} ({}MB)", file.getName(), file.length() / 1024 / 1024);
            return file;
        }
        ...
        try {
            ...
            // 检查解压后的像素数据大小，避免超大分辨率图片导致 OOM
            // 每个像素 ARGB 占用 4 字节
            long pixelBytes = (long) image.getWidth() * image.getHeight() * 4;
            if (pixelBytes > 200L * 1024 * 1024) {
                // 解压后超过 200MB 内存占用，放弃转换
                log.info("跳过 WebP 转换，图片分辨率过大: {}x{} ({}), 预估内存: {}MB",
                        image.getWidth(), image.getHeight(), file.getName(), pixelBytes / 1024 / 1024);
                image.flush();
                return file;
            }
    ...
    private void setFileConfig(String fileId, String username, File file, String fileName, String suffix, String contentType, String relativePath, Update update, Boolean isTogether, String spaceFlag) {
        ...
        if (contentType.contains(Constants.VIDEO)) {
            setMediaCover(fileId, username, fileName, relativePath, update, isTogether, spaceFlag);
        }
        if (contentType.startsWith(Constants.CONTENT_TYPE_IMAGE) && (!"ico".equals(suffix) && !"svg".equals(suffix) && !"dwg".equalsIgnoreCase(suffix))) {
    ...
    private void setMediaCover(String fileId, String username, String fileName, String relativePath, Update update, Boolean isTogether, String spaceFlag) {
        String coverPath = videoProcessService.getVideoCover(fileId, username, relativePath, fileName, isTogether, spaceFlag);
        if (!CharSequenceUtil.isBlank(coverPath)) {
            if (update == null) {
                update = new Update();
            }
            update.set("content", PathUtil.readBytes(Paths.get(coverPath)));
            videoProcessService.convertToM3U8(fileId, username, relativePath, fileName, isTogether, spaceFlag);
     ...
     */
    /**
     * 用户空间占用缓存，避免每次pushMessage都触发MongoDB聚合查询
     * key: userId, value: 空间占用(bytes)
     */
    private static final Cache<String, Long> SPACE_USAGE_CACHE = Caffeine.newBuilder()
    ...
    public void pushMessage(String username, Object message, String url) {
        ...
        if (!CharSequenceUtil.isBlank(userId)) {
            ...
            long takeUpSpace = SPACE_USAGE_CACHE.get(uid, k -> occupiedSpace(k));
    ...
    /**
     * 清除指定用户的空间占用缓存（在文件增删后调用）
     */
    public void invalidateSpaceCache(String userId) {
        SPACE_USAGE_CACHE.invalidate(userId);
    }
    ...
    public long occupiedSpace(String userId) {
        ...
        List<Bson> list = Arrays.asList(
                ...
                match(ne(IUserService.IS_TOGETHER, Boolean.TRUE)),
                match(Filters.or(Filters.exists(SPACE_FLAG, false), Filters.eq(SPACE_FLAG, null))),
     ...
     * @param update       Update
     */
    private void checkOSSPath(String username, String relativePath, String fileName, Update update, Boolean isTogether, String spaceFlag) {
    ...
    public void modifyFile(String username, File file, Boolean isTogether, String spaceFlag) {
        String fileAbsolutePath = file.getAbsolutePath();
        String fileName = file.getName();
        String relativePath = null;
        if (Boolean.TRUE.equals(isTogether)){
    ...
    public static boolean isLock(File file, String rootDir, String username, Boolean isTogether, String spaceFlag) {
        String filePath = getLockFilePath(file, rootDir, username, isTogether, spaceFlag);
        return isLock(filePath);
    }
    ...
    private static String getLockFilePath(File file, String rooDir, String username, Boolean isTogether, String spaceFlag) {
        Path absolutePath = file.toPath();
        Path relativePath = null;
        if (Boolean.TRUE.equals(isTogether)){
            relativePath = absolutePath.subpath(Paths.get(rooDir, "together").getNameCount(), absolutePath.getNameCount());
        }else if (StringUtils.isNotEmpty(spaceFlag)){
            relativePath = absolutePath.subpath(Paths.get(rooDir, spaceFlag).getNameCount(), absolutePath.getNameCount());
        }else{
            relativePath = absolutePath.subpath(Paths.get(rooDir, username).getNameCount(), absolutePath.getNameCount());
        }
